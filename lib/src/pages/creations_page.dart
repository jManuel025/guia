    ...
    return Scaffold(
      ...
      body: Container(
        child: StreamBuilder<QuerySnapshot>(
          stream: Firestore.instance.collection(coleccion).where('usuario_id', isEqualTo: prefs.uid).snapshots(),
            ...
            switch(snapshot.connectionState){
              ...
              default:
                if(snapshot.data.documents.length == 0){
                  return Center(child: Text('No tienes ninguna creación', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 20.0)));
                }
                else{
                  return ListView(
                    children: snapshot.data.documents.map<Widget>(
                      (DocumentSnapshot document){
                        switch (coleccion) {
                          case 'consejos': return Container(
                            child:  _consejo(document, context),
                          );
                          case 'recetas': return Container(
                            child: _receta(document['foto_url'], document['nombreReceta'], document['autor'], "$document", document)
                          );
                          case 'proyectos': return Container(
          ...
          actions: <Widget>[
            ...
            FlatButton(
              ...
              onPressed: () => {
                ...
                Toast.show("Eliminación exitosa", context, duration: Toast.LENGTH_LONG, gravity: Toast.BOTTOM, backgroundColor: Colors.blue)
              }
            ),
          ],
        );
